...
void ParentMainProc(HANDLE hpiShared, HANDLE hevResp)
{
	BYTE arBuff[256];
	char szIn[512];
	...
	try
	{
		...
		if (!bIsChild)
		{
			...
			(
				...
				PIPE_TYPE_BYTE,	1, BUFF_SIZE, 0, INFINITE, NULL
			);
			...
			if (bIsOK)
			{
				...
				ParentMainProc(hpiShared, hevResp);
		...
		else
		{
			...
			(
				SHARE_PIPE, GENERIC_READ, 0, NULL,
			...
			HANDLE arSyncs[3] = { hpiShared, hevResp, hevExit };
