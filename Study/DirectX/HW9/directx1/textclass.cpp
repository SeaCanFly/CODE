...
bool TextClass::SetObjNumber(int objnumber, ID3D11DeviceContext* deviceContext)
{
	char tempString[16];
	char objString[16];
	bool result;
	// Convert the cpu integer to string format.
	_itoa_s(objnumber, tempString, 10);
	// Setup the cpu string.
	strcpy_s(objString, "Obj number: ");
	strcat_s(objString, tempString);
	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence3, objString, 20, 60, 0.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
...
bool TextClass::SetPolyNumber(unsigned long polynumber, ID3D11DeviceContext* deviceContext)
{
	char tempString[32];
	char polyString[32];
	bool result;
	// Convert the cpu integer to string format.
	_ultoa_s(polynumber, tempString, 10);
	// Setup the cpu string.
	strcpy_s(polyString, "Poly: ");
	strcat_s(polyString, tempString);
	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence4, polyString, 20, 80, 0.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
...
bool TextClass::SetScreenPix(int width,int height, ID3D11DeviceContext* deviceContext)
{
	char tempString[16];
	char pixString[16];
	bool result;
	...
	strcpy_s(pixString, "Screen: ");
	strcat_s(pixString, tempString);
	strcat_s(pixString, "X");
	_itoa_s(height, tempString, 10);
	strcat_s(pixString, tempString);
	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence5, pixString, 20, 100, 0.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
