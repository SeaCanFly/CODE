...
bool UserInterfaceClass::Initialize(D3DClass* Direct3D, int screenHeight, int screenWidth, char* minimappath,char* pointpath)
{
	bool result;
	char videoCard[128];
	int videoMemory;
	...
	result = m_Font1->Initialize(Direct3D->GetDevice(), Direct3D->GetDeviceContext(), "Engine/data/font/font01.txt", 
								 "Engine/data/font/font01.tga", 27.0f, 1);
	if (!result)
	{
		return false;
	}
	...
	m_PositionStrings = new TextClass[8];
	...
	result = m_PositionStrings[5].Initialize(Direct3D->GetDevice(), Direct3D->GetDeviceContext(), screenWidth, screenHeight, 16, false, m_Font1, 
											 "rZ: 0", 10, 410, 1.0f, 0.0f, 0.0f);  
	if(!result) 
	{ 
		return false; 
	}
	result = m_PositionStrings[6].Initialize(Direct3D->GetDevice(), Direct3D->GetDeviceContext(), screenWidth, screenHeight, 16, false, m_Font1,
		"Speed: 1", 10, 430, 1.0f, 0.0f, 0.0f);
	if (!result)
	{
		return false;
	}
		result = m_PositionStrings[7].Initialize(Direct3D->GetDevice(), Direct3D->GetDeviceContext(), screenWidth, screenHeight, 16, false, m_Font1,
		"Selected: 1", 10, 700, 1.0f, 0.0f, 0.0f);
	if (!result)
	{
		return false;
	}
	...
	result = m_RenderCountStrings[2].Initialize(Direct3D->GetDevice(), Direct3D->GetDeviceContext(), screenWidth, screenHeight, 32, false, m_Font1, 
												"Cells Culled: 0", 10, 320, 1.0f, 1.0f, 1.0f);
	if(!result)
	{ 
		return false; 
	}
	...
	result = m_MiniMap->Initialize(minimappath,pointpath,Direct3D->GetDevice(), Direct3D->GetDeviceContext(), screenWidth, screenHeight, 1025, 1025);
	if(!m_MiniMap)
	{
		return false;
	}
	m_aspeed = new BitmapClass();
	result = m_aspeed->Initialize(Direct3D->GetDevice(), Direct3D->GetDeviceContext(), screenWidth, screenHeight, 154, 32, "Engine/data/textures/as.tga");
	if (!result)
	{
		printf("m_aspeed initialize failed\n");
		return false;
	}
	m_mspeed = new BitmapClass();
	result = m_mspeed->Initialize(Direct3D->GetDevice(), Direct3D->GetDeviceContext(), screenWidth, screenHeight, 154, 32, "Engine/data/textures/ms.tga");
	if (!result)
	{
		printf("m_aspeed initialize failed\n");
		return false;
	...
	m_ui[0] = new BitmapClass();
	result = m_ui[0]->Initialize(Direct3D->GetDevice(), Direct3D->GetDeviceContext(), screenWidth, screenHeight, 154, 32, "Engine/data/textures/xyz.tga");
	if (!result)
	{
		printf("m_ui[0] initialize failed\n");
		return false;
	}
	m_ui[1] = new BitmapClass();
	result = m_ui[1]->Initialize(Direct3D->GetDevice(), Direct3D->GetDeviceContext(), screenWidth, screenHeight, 154, 32, "Engine/data/textures/wer.tga");
	if (!result)
	{
		printf("m_ui[1] initialize failed\n");
		return false;
	}
		m_ui[2] = new BitmapClass();
	result = m_ui[2]->Initialize(Direct3D->GetDevice(), Direct3D->GetDeviceContext(), screenWidth, screenHeight, 154, 32, "Engine/data/textures/save.tga");
	if (!result)
	{
		printf("m_ui[2] initialize failed\n");
		return false;
	}
		m_ui[3] = new BitmapClass();
	result = m_ui[3]->Initialize(Direct3D->GetDevice(), Direct3D->GetDeviceContext(), screenWidth, screenHeight, 154, 32, "Engine/data/textures/load.tga");
	if (!result)
	{
		printf("m_ui[3] initialize failed\n");
		return false;
	}
...
void UserInterfaceClass::Shutdown()
{
	...
	if(m_PositionStrings)
	{
		...
		m_PositionStrings[6].Shutdown();
		m_PositionStrings[7].Shutdown();
...
bool UserInterfaceClass::Frame(ID3D11DeviceContext* deviceContext, int fps, float posX, float posY, float posZ, 
							   float rotX, float rotY, float rotZ,float speed,int select)
{
	bool result;
	...
	result = UpdatePositionStrings(deviceContext, posX, posY, posZ, rotX, rotY, rotZ,speed,select);
	if(!result)
	{
		return false;
	}
...
bool UserInterfaceClass::Render(D3DClass* Direct3D, ShaderManagerClass* ShaderManager, XMMATRIX worldMatrix, XMMATRIX viewMatrix, 
								XMMATRIX orthoMatrix,int screenW,int screenH)
{
	int i;
	bool result;
	...
	result = m_aspeed->Render(Direct3D->GetDeviceContext(), screenW - 154, 0);
	ShaderManager->RenderTextureShader(Direct3D->GetDeviceContext(), m_aspeed->GetIndexCount(), worldMatrix, viewMatrix, orthoMatrix, m_aspeed->GetTexture());

	result = m_mspeed->Render(Direct3D->GetDeviceContext(), screenW - 154, 32);
	ShaderManager->RenderTextureShader(Direct3D->GetDeviceContext(), m_mspeed->GetIndexCount(), worldMatrix, viewMatrix, orthoMatrix, m_mspeed->GetTexture());
	for (i = 0; i < 4; i++)
	{
		result = m_ui[i]->Render(Direct3D->GetDeviceContext(), screenW - 154, 64 + i * 32);
		ShaderManager->RenderTextureShader(Direct3D->GetDeviceContext(), m_ui[i]->GetIndexCount(), worldMatrix, viewMatrix, orthoMatrix, m_ui[i]->GetTexture());
	}
...
bool UserInterfaceClass::UpdatePositionStrings(ID3D11DeviceContext* deviceContext, float posX, float posY, float posZ, 
											   float rotX, float rotY, float rotZ,float speed,int select)
{
	int positionX, positionY, positionZ, rotationX, rotationY, rotationZ,s,g;
	char tempString[16];
	char finalString[16];
	bool result;
	...
	if (s!= m_previousPosition[6])
	{
		m_previousPosition[6] = s;
		_itoa_s(s, tempString, 10);
		strcpy_s(finalString, "Speed: ");
		strcat_s(finalString, tempString);
		result = m_PositionStrings[6].UpdateSentence(deviceContext, m_Font1, finalString, 10, 240, 1.0f, 1.0f, 0.0f);
		if (!result) { return false; }
	}
	if (g!= m_previousPosition[7])
	{
		m_previousPosition[6] = g;
		_itoa_s(g, tempString, 10);
		strcpy_s(finalString, "Selected: ");
		strcat_s(finalString, tempString);
		result = m_PositionStrings[7].UpdateSentence(deviceContext, m_Font1, finalString, 10, 260, 0.0f, 1.0f, 1.0f);
		if (!result) { return false; }
	}
	return true;
}
...
bool UserInterfaceClass::UpdateRenderCounts(ID3D11DeviceContext* deviceContext, int renderCount, int nodesDrawn, int nodesCulled)
{
	...
	strcpy_s(finalString, "Polys: ");
	strcat_s(finalString, tempString);

	// Update the sentence vertex buffer with the new string information.
	result = m_RenderCountStrings[0].UpdateSentence(deviceContext, m_Font1, finalString, 10, 280, 1.0f, 1.0f, 1.0f);
	if(!result)
	{
		return false;
	}
	...
	strcpy_s(finalString, "Cells: ");
	strcat_s(finalString, tempString);

	// Update the sentence vertex buffer with the new string information.
	result = m_RenderCountStrings[1].UpdateSentence(deviceContext, m_Font1, finalString, 10, 300, 1.0f, 1.0f, 1.0f);
	if(!result)
	{
		return false;
	}
	...
	// Update the sentence vertex buffer with the new string information.
	result = m_RenderCountStrings[2].UpdateSentence(deviceContext, m_Font1, finalString, 10, 320, 1.0f, 1.0f, 1.0f);
	if(!result)
	{
		return false;
	}
