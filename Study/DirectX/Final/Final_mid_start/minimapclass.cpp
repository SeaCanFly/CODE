...
bool MiniMapClass::Initialize(char* filepath,char* pointpath,ID3D11Device* device, ID3D11DeviceContext* deviceContext, int screenWidth, int screenHeight, 
							  float terrainWidth, float terrainHeight)
{
	...
	m_mapLocationX = screenWidth  - m_mapSizeX-10;//screenWidth - (int)m_mapSizeX - 10;
	m_mapLocationY = screenHeight - m_mapSizeY-10;//10;
	...
	result = m_MiniMapBitmap->Initialize(device, deviceContext, screenWidth, screenHeight, 154, 154, filepath);
	if(!result)
	{
		return false;
	}
	...
	result = m_PointBitmap->Initialize(device, deviceContext, screenWidth, screenHeight, 3, 3,pointpath);
	if(!result)
	{
		return false;
	}
