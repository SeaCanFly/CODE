...
void D3DClass::SaveVideoCardInfo()
{
	char memorySize[256] = { 0 };
	char cardName[256] = { 0 };
	int size = 0;
	GetVideoCardInfo(cardName, size);
	FILE* fp = fopen("VideoInfo.txt", "w+");
	sprintf(memorySize, "\nVideo Memory Size:%d\n", size);
	fwrite(cardName, sizeof(char), strlen(cardName) / sizeof(char), fp);
	fwrite(memorySize, sizeof(char), strlen(memorySize) / sizeof(char), fp);
	fclose(fp);
	return;
}
