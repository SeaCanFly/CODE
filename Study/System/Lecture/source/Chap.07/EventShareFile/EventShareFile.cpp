...
VOID CALLBACK ChildTPollProc(PTP_CALLBACK_INSTANCE pInst, PVOID pCtx, PTP_WAIT ptpWait, TP_WAIT_RESULT)
{
	...
	DWORD dwReads = 0;
	ReadFile(hFile, arBuff, dwSize, &dwReads, NULL);
	CloseHandle(hFile);
...
void ParentMainProc(HANDLE hevSend, HANDLE hevResp)
{
	BYTE arBuff[256];
	char szIn[512];
	...
	while (true)
	{
		...
		DWORD dwWrotes = 0;
		WriteFile(hFile, arBuff, lSize + sizeof(LONG) * 2, &dwWrotes, NULL);
		CloseHandle(hFile);
	...
	try
	{
		...
		if (!bIsChild)
		{
			...
			if (bIsOK)
			{
				...
				ParentMainProc(hevSend, hevResp);
		...
		else
		{
			...
			HANDLE arSyncs[2] = { hevSend, hevResp };
