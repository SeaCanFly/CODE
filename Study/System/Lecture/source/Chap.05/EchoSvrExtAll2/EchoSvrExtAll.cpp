...
void _tmain()
{
	...
	while (true)
	{
		DWORD dwWaitRet = WaitForMultipleObjects(2, arSyncObjs, FALSE, 2000);
		...
		{
			EnterCriticalSection(&ie._cs);
			if (ie._pool.size() > POOL_MIN_SIZE)
			{
				SOCK_SET::iterator it = ie._pool.begin();
				PSOCK_ITEM psi = *it;
				ie._pool.erase(it);
				closesocket(psi->_sock);
				delete psi;
				printf("...Timeout expired, pool=%d, conn=%d\n",
					ie._pool.size(), ie._set.size());
			}
			LeaveCriticalSection(&ie._cs);
			continue;
		}
		...
		{
			EnterCriticalSection(&ie._cs);
			if (ie._pool.size() < POOL_MAX_SIZE)
			{
				for (int i = 0; i < PERMIT_INC_CNT; i++)
					PostQueuedCompletionStatus(ie._iocp, 0, IOKEY_INCREASE, NULL);
			}
