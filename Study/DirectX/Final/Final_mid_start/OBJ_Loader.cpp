...
namespace objl
{
	...
	// Structure: Mesh
		...
		//	//result=CreateWICTextureFromFile(device, filename, 0, &m_dtexture, 0);
		//	if (FAILED(result))
		//	{
		//		return false;
		//	}
		...
		bool Mesh::Load(TextureManagerClass* TextureManager,ID3D11Device* device, ID3D11DeviceContext* deviceContext, char* filename)
	{
			bool result;
			printf("%d loading texture\n", TextureManager->m_curCount+1);
			result=TextureManager->LoadTexture(device, deviceContext, filename, TextureManager->m_curCount + 1);
			if (!result)
		{
			
			return false;
		}
			printf("%d loaded texture\n", TextureManager->m_curCount);
			m_textureCount = TextureManager->m_curCount;
		return true;
	}
		void Mesh::Shutdown()
		{
