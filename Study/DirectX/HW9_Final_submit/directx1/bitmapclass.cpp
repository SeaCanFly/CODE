...
void BitmapClass::PositionUpdate(float posx, float posy)
{
	x = posx;
	y = posy;
}
...
bool BitmapClass::Render(ID3D11DeviceContext* deviceContext, int x,int y)
{
	bool result;
	// Re-build the dynamic vertex buffer for rendering to possibly a different location on the screen.
	result = UpdateBuffers(deviceContext,x,y);
	if (!result)
	{
		return false;
	}
...
bool BitmapClass::UpdateBuffers(ID3D11DeviceContext* deviceContext, int x,int y)
{
	...
	//right = left+(float)(m_screenWidth);
