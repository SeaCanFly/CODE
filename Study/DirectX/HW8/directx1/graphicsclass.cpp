...
bool GraphicsClass::Initialize(int screenWidth, int screenHeight, HWND hwnd)
{
	...
	result=m_phone->Initialize(m_D3D->GetDevice(), "Engine/data/wooden watch tower2.obj");
	if (!result)
	{
		MessageBox(hwnd, L"Could not initialize the OBJ", L"Error", MB_OK);
		return false;
	...
	result=m_house->Initialize(m_D3D->GetDevice(), "Engine/data/AWP_Dragon_Lore.obj");
	if (!result)
	{
		MessageBox(hwnd, L"Could not initialize the OBJ", L"Error", MB_OK);
		return false;
	...
	result = m_plane->Initialize(m_D3D->GetDevice(), "Engine/data/gym.obj");
	if (!result)
	{
		MessageBox(hwnd, L"Could not initialize the OBJ", L"Error", MB_OK);
		return false;
...
bool GraphicsClass::Render(float rotation)
{
	...
	if (key == 1)
	{
		...
		for (int i = 0; i < (int)m_tank->loader->LoadedMeshes.size(); i++)
		{
			...
			if (m_tank->loader->LoadedMeshes[i].m_dtexture != nullptr)
			{
				result = m_LightShader->Render(m_D3D->GetDeviceContext(), m_tank->loader->LoadedMeshes[i].m_indexCount, worldMatrix,
					viewMatrix, projectionMatrix, m_tank->loader->LoadedMeshes[i].m_dtexture, m_Light->GetDirection(), m_Light->GetAmbientColor(), m_Light->GetDiffuseColor(), m_Camera->GetPosition(), m_Light->GetSpecularColor(), m_Light->GetSpecularPower());
				if (!result)
				{
					return false;
				}
			...
			{
				result = m_LightShader->ObjRender(m_D3D->GetDeviceContext(), m_tank->loader->LoadedMeshes[i].m_indexCount, worldMatrix,
	...
	/*		D3DXVECTOR4 ka = D3DXVECTOR4(M.MeshMaterial.Ka.X, M.MeshMaterial.Ka.Y, M.MeshMaterial.Ka.Z, 1.0f);
			D3DXVECTOR4 kd = D3DXVECTOR4(M.MeshMaterial.Kd.X, M.MeshMaterial.Kd.Y, M.MeshMaterial.Kd.Z, 1.0f);
			D3DXVECTOR4 ks = D3DXVECTOR4(M.MeshMaterial.Ks.X, M.MeshMaterial.Ks.Y, M.MeshMaterial.Ks.Z, 1.0f);*/
			if (m_plane->loader->LoadedMeshes[i].m_dtexture != nullptr)
			{
				result = m_LightShader->Render(m_D3D->GetDeviceContext(), m_plane->loader->LoadedMeshes[i].m_indexCount, worldMatrix,
					viewMatrix, projectionMatrix, m_plane->loader->LoadedMeshes[i].m_dtexture, m_Light->GetDirection(), m_Light->GetAmbientColor(), m_Light->GetDiffuseColor(), m_Camera->GetPosition(), m_Light->GetSpecularColor(), m_Light->GetSpecularPower());
				if (!result)
				{
					return false;
				}
			...
			{
				result = m_LightShader->ObjRender(m_D3D->GetDeviceContext(), m_plane->loader->LoadedMeshes[i].m_indexCount, worldMatrix,
					viewMatrix, projectionMatrix, m_Light->GetDirection(), m_Light->GetAmbientColor(), m_Light->GetDiffuseColor(), m_Camera->GetPosition(), m_Light->GetSpecularColor(), m_Light->GetSpecularPower());
				if (!result)
				{
					return false;
		...
		D3DXMatrixTranslation(&mo, posX, posY, posZ);
		D3DXMatrixRotationY(&ro, rotation);
		D3DXMatrixScaling(&so, 0.09f, 0.09f, 0.09f);
		worldMatrix = so*ro*mo;
		for (int i = 0; i < (int)m_house->loader->LoadedMeshes.size(); i++)
		{
			...
			//D3DXVECTOR4 kd = D3DXVECTOR4(curMesh.MeshMaterial.Kd.X, curMesh.MeshMaterial.Kd.Y, curMesh.MeshMaterial.Kd.Z, 1.0f);
			//D3DXVECTOR4 ks = D3DXVECTOR4(curMesh.MeshMaterial.Ks.X, curMesh.MeshMaterial.Ks.Y, curMesh.MeshMaterial.Ks.Z, 1.0f);
			if (m_house->loader->LoadedMeshes[i].m_dtexture != nullptr)
			{
				result = m_LightShader->Render(m_D3D->GetDeviceContext(), m_house->loader->LoadedMeshes[i].m_indexCount, worldMatrix,
					viewMatrix, projectionMatrix, m_house->loader->LoadedMeshes[i].m_dtexture, m_Light->GetDirection(), m_Light->GetAmbientColor(), m_Light->GetDiffuseColor(), m_Camera->GetPosition(), m_Light->GetSpecularColor(), m_Light->GetSpecularPower());
				if (!result)
				{
					return false;
				}
			...
			{
				result = m_LightShader->ObjRender(m_D3D->GetDeviceContext(), m_house->loader->LoadedMeshes[i].m_indexCount, worldMatrix,
					viewMatrix, projectionMatrix, m_Light->GetDirection(), m_Light->GetAmbientColor(), m_Light->GetDiffuseColor(), m_Camera->GetPosition(), m_Light->GetSpecularColor(), m_Light->GetSpecularPower());
				if (!result)
				{
					return false;
		...
		for (int i = 0; i < (int)m_phone->loader->LoadedMeshes.size(); i++)
		{
			...
			D3DXVECTOR4 kd = D3DXVECTOR4(curMesh.MeshMaterial.Kd.X, curMesh.MeshMaterial.Kd.Y, curMesh.MeshMaterial.Kd.Z, 1.0f);
			D3DXVECTOR4 ks = D3DXVECTOR4(curMesh.MeshMaterial.Ks.X, curMesh.MeshMaterial.Ks.Y, curMesh.MeshMaterial.Ks.Z, 1.0f);*/
			if (m_phone->loader->LoadedMeshes[i].m_dtexture != nullptr)
			{
				result = m_LightShader->Render(m_D3D->GetDeviceContext(), m_phone->loader->LoadedMeshes[i].m_indexCount, worldMatrix,
					viewMatrix, projectionMatrix, m_phone->loader->LoadedMeshes[i].m_dtexture, m_Light->GetDirection(), m_Light->GetAmbientColor(), m_Light->GetDiffuseColor(), m_Camera->GetPosition(), m_Light->GetSpecularColor(), m_Light->GetSpecularPower());
				if (!result)
				{
					return false;
				}
			...
			{
				result = m_LightShader->ObjRender(m_D3D->GetDeviceContext(), m_phone->loader->LoadedMeshes[i].m_indexCount, worldMatrix,
					viewMatrix, projectionMatrix, m_Light->GetDirection(), m_Light->GetAmbientColor(), m_Light->GetDiffuseColor(), m_Camera->GetPosition(), m_Light->GetSpecularColor(), m_Light->GetSpecularPower());
				if (!result)
				{
					return false;
